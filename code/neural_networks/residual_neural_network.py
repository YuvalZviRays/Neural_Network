...
class Residual_Neural_Network(Neural_Network):
    ...
    def jackTMV(self, delta):
        ...
        for i in range(len(self.hidden_layers)-1, -1, -1):
            ...
            grad_w_1 = np.dot(hidden_layer.weight_matrix_2)
            grad_w_2 = np.dot(hidden_layer.weight_matrix_2, np.diag(hidden_layer.derivative()))
            grad_b = np.dot(hidden_layer.weight_matrix_2, np.diag(hidden_layer.derivative()))
            jack.append(grad_b.ravel())
            jack.append(grad_w_1.ravel())
            jack.append(grad_w_2.ravel())
